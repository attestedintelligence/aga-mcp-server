...
This codebase is:
- The working code behind two NIST public comments
- A live MCP server any AI agent (any compatible client, GPT, etc.) can connect to via MCP client or any MCP client
...
│   └── mcp-client-config-local.json     Resolved absolute path
│
├── package.json
├── tsconfig.json
├── vitest.config.ts
...
| 3 | `init_chain` | Initialize continuity chain with genesis event |
| 4 | `attest_subject` | Hash content, attest, seal, generate signed artifact, load into portal. Accepts optional `behavioral_baseline` |
| 5 | `verify_chain` | Verify chain integrity (leaf hashes, linkage, payload hashes) |
| 6 | `list_claims` | List available claims with sensitivity levels |
| 7 | `measure_behavior` | Measure behavioral patterns - unauthorized tools, rate violations, forbidden sequences |
...
| # | Tool | Description |
|---|---|---|
| 10 | `measure_integrity` | Measure content against sealed hash, enforce on drift, generate receipt |
| 11 | `revoke_artifact` | Mid-session artifact revocation, pushes REVOCATION chain event |
| 12 | `create_checkpoint` | Build Merkle tree over chain events, produce checkpoint |
| 13 | `generate_evidence_bundle` | Package artifact + receipts + Merkle proofs for offline verification |
| 14 | `verify_bundle_offline` | 4-step offline verification (artifact sig, receipt sigs, Merkle proofs, anchor) |
| 15 | `request_claim` | Privacy-preserving disclosure with sensitivity-based auto-substitution |
| 16 | `delegate_to_subagent` | Derive constrained artifact for sub-agent (scope only diminishes) |
...
### 5.3 Leaf Hash (Privacy Innovation)
...
**The actual payload is EXCLUDED from the leaf hash.** This is a key privacy innovation - chain integrity can be verified without revealing the contents of any event. Only a hash of the payload is included, preserving privacy while maintaining tamper evidence.
...
| `core/chain.test.ts` | 7 | Genesis sequence, increment, intact chain, tampered leaf/payload, **leaf excludes payload**, REVOCATION event |
...
|---|---|---|
| `v0.1.0` | `62394ed` | Initial reference implementation - 45 files, 45 tests |
| (v0.1.1) | `1093631` | Hardening - .npmignore, LICENSE, keypair gen, benchmark, MCP client config |
...
- [x] MCP client config generated with absolute path
...
- [x] Continuity chain with privacy-preserving leaf hashes
...
| Item | Action | Why |
|---|---|---|
| **npm publish** | Run `npm login` then `npm publish --access public` in terminal | Publishes to npm registry. Requires interactive 2FA. |
| **MCP client smoke test** | Copy config to your MCP client's config directory, restart the client, run test sequence | Proves the MCP server works as a live tool for AI agents |

### Near-Term Development

| Priority | Feature | Description |
|---|---|---|
| HIGH | Arweave Anchoring | Replace `SKIPPED_OFFLINE` stub with real blockchain anchoring. POST Merkle root to Arweave, store transaction IDs, enable Step 4 of offline verification. |
| HIGH | SPIFFE/SPIRE Integration | SPIRE handles workload-to-node identity (SVID), AGA handles workload-to-intent governance. Integration point: SVID provides transport identity, AGA binds governance. |
| MEDIUM | Multi-Agent Chain Linking | Child agent's genesis event links to parent's chain. Cross-chain verification for delegation audit trails. |
...
2. Copy config:
   - From: `config/mcp-client-config-local.json`
   - To: your MCP client's config directory
...
| Leaf hash excludes payload | Privacy innovation - enables chain verification without revealing event contents |
