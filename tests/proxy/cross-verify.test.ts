...
describe('Cross-Verification: Proxy → Bundle → Verifier', () => {
  ...
  it('proxy bundle verifies under the canonical SEP verifier (pinned)', () => {
    const bundle = proxy.exportBundle();
    // The proxy bundle is verified by the ONE canonical, sound verifier (src/sep),
    // pinned to the proxy's gateway key — not a proxy-local verifier.
