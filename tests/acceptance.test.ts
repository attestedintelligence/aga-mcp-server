...
describe('Group 4: Continuity Chain', () => {
  ...
  // (4.4 ANCHOR_BATCH test removed in P4 — it exercised the deleted legacy continuity checkpoint.)
...
describe('Group 8: End-to-End Scenarios', () => {
  ...
  it('8.2 Drone - Safe-state transition', () => {
    ...
    expect(verifyEvidenceBundle(JSON.stringify(result.sepBundle), result.sepBundle.public_key).verdict).toBe('VERIFIED');
...
describe('Group 9: Independent Verifier Proof', () => {
  ...
  it('9.2: Independent verifier reaches same conclusion as AGA verifier', () => {
    ...
    // Independent SEP verifier (zero AGA imports) says VERIFIED on the canonical SEP bundle
    const indResult = verifyEvidenceBundle(JSON.stringify(scada.sepBundle), scada.sepBundle.public_key);
    expect(indResult.verdict).toBe('VERIFIED');
    expect(indResult.issuerVerified).toBe(true);
  });

  it('9.3: Independent verifier detects tampered bundle', () => {
    const scada = runScadaScenario();
    const bundle = scada.sepBundle;

    // Tamper a receipt signature (still valid hex, wrong signature)
    const tampered = {
      ...bundle,
      receipts: bundle.receipts.map((r, i) => i === 0 ? { ...r, signature: (r.signature[0] === '0' ? '1' : '0') + r.signature.slice(1) } : r),
    };

    const indResult = verifyEvidenceBundle(JSON.stringify(tampered), bundle.public_key);
    expect(indResult.verdict).toBe('FAILED');
  });

  it('9.4: Independent verifier works on all 3 scenario bundles', () => {
    const s1 = runScadaScenario();
    expect(verifyEvidenceBundle(JSON.stringify(s1.sepBundle), s1.sepBundle.public_key).verdict).toBe('VERIFIED');

    const s2 = runAutonomousVehicleScenario();
    expect(verifyEvidenceBundle(JSON.stringify(s2.sepBundle), s2.sepBundle.public_key).verdict).toBe('VERIFIED');

    const s3 = runAiAgentScenario();
    expect(verifyEvidenceBundle(JSON.stringify(s3.sepBundle), s3.sepBundle.public_key).verdict).toBe('VERIFIED');
  });
});
