...
describe('portal', () => {
  ...
  // D1 (2026-08-29): TTL expiry fails closed. The degradation entry is still written — it is the
  // forensic record of WHY the portal terminated — but the state is TERMINATED, matching revocation.
  it('TTL expiry → TERMINATED (fail closed), with the degradation reason still recorded', () => { const p = new Portal(); p.loadArtifact(mk(0), pkToHex(kp.publicKey)); const r = p.measure(enc.encode(content), meta); expect(r.ttl_ok).toBe(false); expect(r.degraded).toBe(true); expect(p.state).toBe('TERMINATED'); expect(p.degradationLog).toHaveLength(1); expect(p.degradationLog[0].reason).toBe('TTL_EXPIRED'); });
  // Replaces 'SAFE_STATE allows continued measurement'. Continued measurement past expiry is precisely
  // the behaviour D1 removed, so the invariant is now the opposite one: the channel is closed.
  it('after TTL expiry the portal refuses further measurement (no second receipt)', () => { const p = new Portal(); p.loadArtifact(mk(0), pkToHex(kp.publicKey)); p.measure(enc.encode(content), meta); expect(p.state).toBe('TERMINATED'); expect(() => p.measure(enc.encode(content), meta)).toThrow(/terminated/i); expect(p.degradationLog).toHaveLength(1); });
