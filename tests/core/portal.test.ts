...
describe('portal', () => {
  ...
  it('TTL expiry → TERMINATED', () => { const p = new Portal(); p.loadArtifact(mk(0), pkToHex(kp.publicKey)); expect(p.measure(enc.encode(content), meta).ttl_ok).toBe(false); expect(p.state).toBe('TERMINATED'); });
