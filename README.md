...
10. **Policy constraints check less than their names suggest.** A `path_prefix` is checked only when the value under the
    ...
    `allowed: "false"`, a string, allows the tool, and a `path_keys` string instead of an array turns the path check off. Rate limits count per tool name across the whole
    ...
    nothing; `allowed: "false"` forwarded the call in both modes. Workaround: treat path and pattern rules as a convenience rather than a boundary, restrict paths in the
    ...
    release.
11. **A result from a stdio upstream longer than 8,388,608 characters (UTF-16 code units) is dropped.** The call already
    ...
    result came back in 31 milliseconds. Workaround: keep tool results under the bound, for example by reading large files
