...
- **Reaching the live ledger:** the proxy holds its ledger in-process. `aga-proxy start` opens a **loopback-only control channel** (`127.0.0.1`, own port, read routes only) so a **separate** `aga-proxy export` invocation can fetch the live signed bundle while the proxy is up; with no proxy running, export fails loudly rather than emitting an empty bundle. This closes the earlier "no documented export path" gap. It changes **reachability only** — it does **not** add restart persistence: the in-memory property above still holds. The channel is bound to loopback and read-only (it cannot mutate policy or forge a receipt; the artifact is already signed), but it does not check a request's Host or Origin header, so a web page in a browser on the same host can read the live receipts and bundle from it through DNS rebinding unless the browser blocks it (README known issue 12). One honest exposure note for a genuinely multi-user host: while the proxy is running, any local user on that host can read the signed ledger over the loopback channel, which may exceed the file-permission scope of an exported bundle on disk. For a shared-host pilot, prefer local authorization (an owner-readable token, or a unix-domain socket with filesystem permissions) over TCP loopback.
- **Scale:** a compact bundle (the control channel's `GET /export`) takes about 1.5 to 2 KB per receipt, and the indented file `aga-proxy export` writes about 2.1 KB at 1,000 receipts and 2.5 KB at 11,000, both rising slowly with the count, because each Merkle inclusion proof is `O(log n)`. In 3.6.0 through 3.6.2, export time grows with the square of the receipt count and blocks governed calls while it runs (the README's known issue 9). Very large or long-lived chains, and high-throughput retention, are bounded by process memory in the default in-memory mode and want the persistent backend. AGA has not been benchmarked as a high-volume datastore; it is an evidence format + governance interception point, not a log warehouse.

## What `policy_reference` binds — and does not

`policy_reference` is one of the signed receipt fields, and what it holds depends on the binary: **aga-proxy** signs into every receipt the SHA-256 of its policy's canonical JSON (the same canonicalization as receipts, so reformatting the policy file does not change it; recompute it with `derivePolicyReference` from `@attested-intelligence/aga-mcp-server/sep`), while **aga-mcp-server 3.6.0** signs an empty `policy_reference`, so its receipts do not identify a policy. The bundle *envelope* also carries an **unsigned** `policy_reference` (plus `bundle_id`, `schema_version`, `offline_capable`) as a convenience mirror — these four envelope fields have no signed counterpart and must not be trusted as security-identity values. The security-identity fields `gateway_id` / `merkle_root` / `generated_at` **are** bound (envelope-consistency check). Binding the envelope `policy_reference` is a recommended near-term (3.1) format revision. A relying party must trust only signed/verified values and pin the gateway key. (`THREAT_BOUNDARY.md` §3.8.)
