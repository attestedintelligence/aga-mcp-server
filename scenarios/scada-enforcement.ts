...
// legacy continuity bundle/checkpoint imports removed (P4) — scenario emits only the canonical SEP bundle
...
import { projectSepBundle } from './sep-bundle.js';
import type { SepBundle } from '../src/sep/index.js';
...
export function runScadaScenario(): ScenarioResult {
  ...
  // Canonical SEP projection of the same governance decisions (clean -> PERMITTED, drift -> DENIED)
  const sepBundle = projectSepBundle('scada-scenario', portalKP.secretKey, receipts);

  return { sepBundle, chain: chainEvents };
}
...
if (process.argv[1]?.includes('scada-enforcement')) {
  ...
  writeFileSync(resolve(outDir, 'scada-bundle.json'), JSON.stringify(result.sepBundle, null, 2));
  console.log('SCADA scenario complete. SEP bundle written to scenarios/output/scada-bundle.json');
  console.log(`  receipts: ${result.sepBundle.receipts.length} | merkle_root: ${result.sepBundle.merkle_root.slice(0, 16)}...`);
}
