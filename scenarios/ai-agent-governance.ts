...
export function runAiAgentScenario(): ScenarioResult {
  ...
  const sepBundle = projectSepBundle('ai-agent-scenario', portalKP.secretKey, receipts);
...
if (process.argv[1]?.includes('ai-agent-governance')) {
  ...
  writeFileSync(resolve(outDir, 'agent-bundle.json'), JSON.stringify(result.sepBundle, null, 2));
  console.log('AI Agent scenario complete. SEP bundle written to scenarios/output/agent-bundle.json');
