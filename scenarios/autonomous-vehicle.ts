...
export function runAutonomousVehicleScenario(): ScenarioResult {
  ...
  const sepBundle = projectSepBundle('vehicle-scenario', portalKP.secretKey, receipts);
...
if (process.argv[1]?.includes('autonomous-vehicle')) {
  ...
  writeFileSync(resolve(outDir, 'vehicle-bundle.json'), JSON.stringify(result.sepBundle, null, 2));
  console.log('Autonomous Vehicle scenario complete. SEP bundle written to scenarios/output/vehicle-bundle.json');
